...
Lists can be created by adding places individually via search or in bulk using formatted JSON or uploading a CSV.
...
### Google Places API key (prod)

The browser calls `places.googleapis.com` with your key. In [Google Cloud Console](https://console.cloud.google.com/) → **APIs & Services** → **Credentials** → your browser key → **Application restrictions** → **HTTP referrers**, add every origin users hit (each line is one pattern):

- `https://www.top100sf.com/*`
- `https://top100sf.com/*`
- `http://localhost:*` (optional, for local dev)

If you only allow `https://top100sf.com/*` but the site is opened as `https://www.top100sf.com/`, Google returns **403** with `Requests from referer https://www.top100sf.com/ are blocked`.

Ensure **Places API (New)** is enabled for the project. The key is injected at build time via `GOOGLE_PLACES_API_KEY` in `.env` when you run `npm run dist` / `npm run deploy` (see `scripts/build-dist.mjs`).
